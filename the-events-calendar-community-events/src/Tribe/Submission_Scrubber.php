...
class Tribe__Events__Community__Submission_Scrubber {
	...
	/**
	 * Sets the series to attach to the newly created event into the submission array
	 *
	 * @since 4.10.0
	 */
	protected function set_series() {
		if ( ! isset( $this->submission['series']['id'] ) ) {
			$this->submission['Series'] = 0;

			return;
		}

		$this->submission['Series'] = (int) array_shift($this->submission['series']['id'] );
		unset( $this->submission['series'] );
	}
