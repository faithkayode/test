...
if ( ! class_exists( 'Tribe__Events__Community__Main' ) ) {
	...
	class Tribe__Events__Community__Main {
		...
		public function filter_document_title( $title_parts ) {
			...
			_deprecated_function( __METHOD__, '4.10.9','No replacement.' );
		...
		/**
		 * Custom template for Community Events
		 *
		 * @since 4.10.0
		 *
		 * @TODO  Update method to be more "V2".
		 *
		 * @param string $name Name of the route the template is used for.
		 *
		 * @return string Template Path.
		 */
		public function getTemplateFile( $name ) {
			$template_path = Tribe__Events__Templates::getTemplateHierarchy( 'community/community_default_view.php' );

			/**
			 * Allows customizing the template per a route
			 *
			 * Default is the base template
			 *
			 * @since 4.10.0
			 *
			 * @param string $template_path The URL to redirect to.
			 * @param string $name          Name of the route currently on.
			 *
			 * @return string
			 */
			return apply_filters( 'tribe_events_community_template', $template_path, $name );
		...
		public function doEventForm( $id = null, $shortcode = false ) {
			/**
			 * Filters the post ID the Event Form is rendering for.
			 *
			 * @since 4.10.5
			 *
			 * @param int|null $id The post ID the Event Form is rendering for.
			 */
			$id = apply_filters( 'tec_events_community_event_form_post_id', $id );
			...
			if ( $tribe_event_id && class_exists( 'Tribe__Events__Pro__Main' ) && tribe_is_recurring_event( $tribe_event_id ) ) {
				$this->enqueueOutputMessage( sprintf( __( '%sWarning:%s You are editing a recurring %3$s. Changes will be applied to all occurrences of this %3$s.', 'tribe-events-community' ), '<b>', '</b>', $events_label_singular_lowercase ), 'error' );
			}
			...
			// security check
			if ( $edit && $tribe_event_id && ! ( current_user_can( 'edit_post', $tribe_event_id ) || $this->user_can_edit_their_submissions($tribe_event_id) ) ) {
			...
			if ( $this->allowAnonymousSubmissions || is_user_logged_in() ) {
				...
				if ( ! empty( $submission ) ) {
					// If a Post ID was sent over, let's validate the post before moving forward.
					if ( isset( $submission['post_ID'] ) ) {

						$auto_draft_event = get_post( absint( $submission['post_ID'] ) );

						// Confirm the post matches an event.
						if ( $auto_draft_event instanceof \WP_Post
							 && $auto_draft_event->post_type === Tribe__Events__Main::POSTTYPE
							 && get_current_user_id() == $auto_draft_event->post_author
						) {
					...
					if ( $handler->validate() ) {
						...
						/**
						 * Allow the user to add logic after the form is submitted and validated.
						 * For example, redirecting after form submission.
						 *
						 * @since 4.10.5
						 */
		...
		public function doMyEvents( $page = null, $print_before_after_override = false, $shortcode = false ) {
			...
			if ( is_user_logged_in() ) {
				...
				if ( empty( $_GET['eventDisplay'] ) || 'list' === $_GET['eventDisplay'] ) {
					// Anything that ends after now is considered "Upcoming".
					$args['ends_after'] = 'now';
				} else if ( 'past' === $_GET['eventDisplay'] ) {
