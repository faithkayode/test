...
class Tribe__Events__Community__Submission_Handler {
	...
	public function validate() {
		...
		if ( $this->event_id ) {
			...
			$is_draft                        = $event->post_status === 'auto-draft';
			$user_can_edit                   = $this->community->user_can_edit( $this->event_id, \Tribe__Events__Main::POSTTYPE );
			$user_can_edit_their_submission  = $this->community->user_can_edit_their_submissions( $this->event_id );
			$is_editing_enabled_for_user     = $user_can_edit || $user_can_edit_their_submission;
			$events_label_singular_lowercase = tribe_get_event_label_singular_lowercase();

			// If the event is a draft, and you are not an author, turn valid to false.
			if ( $is_draft && ! $is_author ) {
				$this->valid = false;
			...
			// If the event is not a draft, and editing is disabled, turn valid to false.
			} elseif ( ! $is_draft && ! $is_editing_enabled_for_user ) {
				$this->valid = false;
				$this->add_message( esc_html( sprintf( __( 'There was a problem saving your %s, please try again.', 'tribe-events-community' ), $events_label_singular_lowercase ) ), 'error' );
			}
		}
	...
	public function save() {
		...
		$this->submission = $this->sanitize_linked_post( 'series', $this->submission );
		...
		if ( ! empty( $this->submission['Series'] ) || function_exists( 'tribe_update_event_with_series' ) ) {
			$new_event = get_post( $this->event_id );
			$series    = get_post( $this->submission['Series'] );
		...
		switch ( $key ) {
			...
			case 'venue' :
				...
				foreach ( $submission['Venue']['VenueID'] as $key => $venue_id ) {
					// We have an ID for an existing Venue.
					if ( 0 < $venue_id ) {
						return true;
					}
					// When `Users cannot create new Venues` is enabled the $venue_id is `-1`
					// We also make sure the Venue Name is empty as an extra check to make sure
					// it's not considered a new venue.
					elseif ( $venue_id === - 1 && empty( $submission['Venue']['Venue'][ $key ] ) ) {
						return false;
					}
				}
				/*
				...
				foreach ( $submission['Venue'] as $venue_field_name => $venue_field_value ) {
					 ...
					 *
					 * @param array of fields to validate - Venue, Address, City, Country, Province, State, Zip, Phone, URL
					 */
