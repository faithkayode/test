...
(function ( window, $, obj ) {
	...
	obj.on_change_datepicker = function() {
		...
		var other_date = $.datepicker.parseDate(
			instance.settings.dateFormat || $.datepicker._defaults.dateFormat,
			$other.val(),
			instance.settings
		);
	...
	obj.init_list_columns_menu = function () {
		...
		$( '.table-menu-btn' ).on( 'click', function () {
			...
			clickedItem
				...
				.parent()
				.find( '.table-menu' ) // eslint-disable-line es5/no-es6-methods
	...
	obj.is_form_field_empty = function( field_name, form_state ) {
		...
		for ( var i = 0; i < form_state.length; i++ ) {
			...
			if ( field_name == form_state[ i ].name ) { // eslint-disable-line eqeqeq
	...
	obj.disallow_submission_while_creating_linked_posts = function() {
		...
		$( '#tribe-community-events form' ).on( 'submit', function( e ) { // eslint-disable-line no-unused-vars,max-len
			var $form        = $( this );
			var error_fields = [];

			$form.find( '.required' ).each( function( index ) { // eslint-disable-line no-unused-vars,max-len
